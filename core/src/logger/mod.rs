...
use crate::sync::{ConstMutex, LazyLock, Mutex, MutexError};
...
pub fn init(channels: impl Into<FlagSet<Channel>>) -> Result<(), LoggerError> {
    ...
    unsafe {
        if new.contains(Channel::Cafe) && sys::WHBLogCafeInit() == 0 {
            return Err(LoggerError::CafeFailed);
        }
        if new.contains(Channel::Console) {
            if sys::WHBLogConsoleInit() != 0 {
                return Err(LoggerError::ConsoleFailed);
            } else {
                sys::WHBLogConsoleSetColor(0xFF000000);
            }
        }
        if new.contains(Channel::Module) && sys::WHBLogModuleInit() == 0 {
            return Err(LoggerError::ModuleFailed);
        }
        if new.contains(Channel::Udp) && sys::WHBLogUdpInit() == 0 {
...
pub fn print(text: &str) -> Result<(), LoggerError> {
    ...
    if logger.channels.is_empty() {
        ...
        unsafe {
            sys::WHBLogPrint(text.as_ptr());

            if logger.channels.contains(Channel::Console) {
                sys::WHBLogConsoleDraw();
            }
            if logger.channels.contains(Channel::Cafe) {
                sys::OSReport(text.as_ptr());
            }
        }
        Ok(())
    }
