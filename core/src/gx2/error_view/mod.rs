...
impl Into<sys::nn_erreula_ErrorType::Type> for Format {
    fn into(self) -> sys::nn_erreula_ErrorType::Type {
        use sys::nn_erreula_ErrorType as T;
        match self {
            Self::Code => T::Code,
...
impl Into<sys::nn_erreula_RenderTarget::Type> for Target {
    fn into(self) -> sys::nn_erreula_RenderTarget::Type {
        use sys::nn_erreula_RenderTarget as T;
        match self {
            Self::Tv => T::Tv,
            Self::Drc => T::Drc,
            Self::Both => T::Both,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            arg: sys::nn_erreula_AppearArg::default(),
            message: vec![0],
            ok_label: vec![0],
            cancel_label: vec![0],
            title: vec![0],
...
impl Config {
    ...
    pub fn get_raw(&self) -> &sys::nn_erreula_AppearArg {
        &self.arg
    }

    pub fn get_raw_mut(&mut self) -> &sys::nn_erreula_AppearArg {
        &mut self.arg
    }
...
impl ErrorView {
    pub fn new(region: Region, language: Language) -> Result<Self, ()> {
        ...
        let mut mem = Vec::with_capacity(unsafe { sys::nn_erreula_GetWorkMemorySize() } as usize);

        let create_arg = sys::nn_erreula_CreateArg {
        ...
        if !unsafe { sys::nn_erreula_Create(&create_arg) } {
            Err(())
        } else {
            Ok(Self {
                _fs: fs,
    ...
    pub fn appear(&self, config: Config) -> ErrorViewRenderer {
        unsafe { sys::nn_erreula_AppearErrorViewer(config.get_raw()) }

        unsafe {
            sys::nn_erreula_PlayAppearSE(config.sound);
        }
...
impl<'a> ErrorViewRenderer<'a> {
    ...
    pub fn update(&self) {
        ...
        unsafe {
            sys::nn_erreula_Calc(&mut info.as_erreula());
        }
    }

    pub fn is_ok(&self) -> bool {
        unsafe { sys::nn_erreula_IsDecideSelectLeftButtonError() }
    }

    pub fn is_cancel(&self) -> bool {
        unsafe { sys::nn_erreula_IsDecideSelectRightButtonError() }
    }

    pub fn get_value(self) -> (i32, i32) {
        unsafe {
            (
                sys::nn_erreula_GetResultType(),
                sys::nn_erreula_GetResultCode(),
            )
        }
    }
}
...
impl<'a> Drop for ErrorViewRenderer<'a> {
    fn drop(&mut self) {
        unsafe {
            sys::nn_erreula_DisappearErrorViewer();
        }
    }
}
