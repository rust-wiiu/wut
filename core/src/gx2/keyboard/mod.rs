...
impl Into<sys::nn_swkbd_KeyboardMode::Type> for Mode {
    fn into(self) -> sys::nn_swkbd_KeyboardMode::Type {
        use sys::nn_swkbd_KeyboardMode as T;
        match self {
            Self::Full => T::Full,
...
impl Into<sys::nn_swkbd_PasswordMode::Type> for PasswordMode {
    fn into(self) -> sys::nn_swkbd_PasswordMode::Type {
        use sys::nn_swkbd_PasswordMode as T;
        match self {
            Self::Clear => T::Clear,
            Self::Hide => T::Hide,
            Self::Fade => T::Fade,
...
impl Default for Config {
    fn default() -> Self {
        Self {
            arg: sys::nn_swkbd_AppearArg::default(),
            ok_string: vec![0],
            initial_text: vec![0],
            hint_text: vec![0],
        }
...
impl Config {
    ...
    pub fn get_raw(&self) -> &sys::nn_swkbd_AppearArg {
        &self.arg
    }

    pub fn get_raw_mut(&mut self) -> &sys::nn_swkbd_AppearArg {
        &mut self.arg
    }
...
impl Keyboard {
    pub fn new(region: Region) -> Result<Self, KeyboardError> {
        ...
        let mut fs = sys::FSClient::default();
        unsafe {
            sys::FSAddClient(&mut fs, sys::FSErrorFlag::FS_ERROR_FLAG_NONE);
        }

        let mut mem = Vec::with_capacity(unsafe { sys::nn_swkbd_GetWorkMemorySize(0) } as usize);

        let create_arg = sys::nn_swkbd_CreateArg {
        ...
        if !unsafe { sys::nn_swkbd_Create(&create_arg) } {
    ...
    pub fn appear(&self, config: Config) -> Result<KeyboardRenderer, ()> {
        let success = unsafe { sys::nn_swkbd_AppearInputForm(config.get_raw()) };

        if success {
            unsafe {
                sys::nn_swkbd_SetCursorPos(config.initial_text.len() as i32);
            }
...
impl<'a> KeyboardRenderer<'a> {
    ...
    pub fn update(&self) {
        ...
        unsafe {
            sys::nn_swkbd_Calc(&mut info.as_swkbd());

            if sys::nn_swkbd_IsNeedCalcSubThreadFont() {
                sys::nn_swkbd_CalcSubThreadFont();
            }

            if sys::nn_swkbd_IsNeedCalcSubThreadPredict() {
                sys::nn_swkbd_CalcSubThreadPredict();
            }
        }
    }

    pub fn is_ok(&self) -> bool {
        unsafe { sys::nn_swkbd_IsDecideOkButton(core::ptr::null_mut()) }
    }

    pub fn is_cancel(&self) -> bool {
        unsafe { sys::nn_swkbd_IsDecideCancelButton(core::ptr::null_mut()) }
    }

    pub fn get_value(self) -> String {
        let value = unsafe { sys::nn_swkbd_GetInputFormString() };
        from_utf16(value)
    }
}
...
impl<'a> Drop for KeyboardRenderer<'a> {
    fn drop(&mut self) {
        unsafe {
            sys::nn_swkbd_DisappearInputForm();
            sys::nn_swkbd_DisappearKeyboard();
        }
    }
}
