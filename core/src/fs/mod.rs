...
pub struct FsHandler {
    ...
    pub client: Box<RefCell<sys::FSClient>>,
    pub block: Box<RefCell<sys::FSCmdBlock>>,
    pub error_mask: sys::FSErrorFlag::Type,
    _resource: RrcGuard,
}
...
impl FsHandler {
    pub fn new() -> Result<Self, FilesystemError> {
        let fs = Self {
            client: Box::new(RefCell::new(sys::FSClient::default())),
            block: Box::new(RefCell::new(sys::FSCmdBlock::default())),
            error_mask: sys::FSErrorFlag::FS_ERROR_FLAG_ALL,
            _resource: FS.acquire(),
        };

        let status = unsafe { sys::FSAddClient(&mut *fs.client.borrow_mut(), fs.error_mask) };

        FilesystemError::try_from(status)?;

        unsafe {
            sys::FSInitCmdBlock(&mut *fs.block.borrow_mut());
        }
...
impl Drop for FsHandler {
    fn drop(&mut self) {
        unsafe { sys::FSDelClient(&mut *self.client.borrow_mut(), self.error_mask) };
    }
}
...
flags! {
    enum MetadataFlags: sys::FSStatFlags::Type {
        ...
        Link = sys::FSStatFlags::FS_STAT_LINK
    }
...
impl From<sys::FSMode::Type> for Permissions {
    fn from(value: sys::FSMode::Type) -> Self {
        use sys::FSMode as M;
...
impl Into<sys::FSMode::Type> for Permissions {
    fn into(self) -> sys::FSMode::Type {
        use sys::FSMode as M;
...
#[derive(Clone, Copy)]
pub struct Metadata(sys::FSStat);
...
impl Metadata {
    ...
    pub fn created(&self) -> Result<DateTime, FilesystemError> {
        ...
        unsafe {
            sys::FSTimeToCalendarTime(self.0.created, &mut cal);
        }
    ...
    pub fn modified(&self) -> Result<DateTime, FilesystemError> {
        let mut cal = sys::OSCalendarTime::default();
        unsafe {
            sys::FSTimeToCalendarTime(self.0.modified, &mut cal);
        }
        Ok(DateTime::from(cal))
    }
...
impl From<sys::FSStat> for Metadata {
    fn from(value: sys::FSStat) -> Self {
        Self(value)
    }
}
...
impl OpenOptions {
    ...
    pub fn open<P: AsRef<Path>>(&self, path: P) -> Result<File, FilesystemError> {
        ...
        let mut handle = sys::FSFileHandle::default();
...
impl FsBuffer {
    fn new<T: Into<u32>>(len: T) -> Self {
        let len = len.into();
        Self {
            data: unsafe { sys::MEMAllocFromDefaultHeapEx.unwrap()(len, 0x40) } as *mut _,
            len,
        }
    }
...
impl Drop for FsBuffer {
    fn drop(&mut self) {
        unsafe {
            sys::MEMFreeToDefaultHeap.unwrap()(self.data as *mut _);
        }
    }
}
...
impl File {
    ...
    pub fn metadata(&self) -> Result<Metadata, FilesystemError> {
        let mut stat = sys::FSStat::default();
...
impl Iterator for ReadDir {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let mut entry = sys::FSDirectoryEntry::default();
...
pub fn read_dir<'a, P: AsRef<Path>>(path: P) -> Result<ReadDir, FilesystemError> {
    ...
    let fs = FsHandler::new()?;
    let mut handle = sys::FSDirectoryHandle::default();
