...
impl From<c_wut::OSCalendarTime> for DateTime {
    fn from(value: c_wut::OSCalendarTime) -> Self {
        Self {
            sec: value.tm_sec,
            min: value.tm_min,
            hour: value.tm_hour,
...
impl From<c_wut::OSTime> for DateTime {
    fn from(value: c_wut::OSTime) -> Self {
        let mut cal = c_wut::OSCalendarTime::default();
        unsafe {
            c_wut::OSTicksToCalendarTime(value, &mut cal);
        }
        Self::from(cal)
    }
}
